...
添加本地 Workspace 只有一条路径：通过组合的目录流程选一个宿主机目录，新建的或已存在的都可以。入口是 `menu.addWorkspace`（「添加本地工作区…」/ "Add local workspace…"）；按名称创建的对话框及其 `create.*`／`menu.createWorkspace`／`workspace.new` 文案全部删除。「本地」用于区分这项带路径的操作与 Provider 持有的远程根；后者保持为选择项，不会被接纳成 Workspace。

**菜单的存在是为了在多个目标之间消歧。** 仅添加的侧边栏表层会把可交互的 Provider 远程根列在本地添加操作旁，主视觉区还可以列出已有本地 Workspace。只剩本地添加操作时，锚点手势*就是*该动作：直接拉起流程，不渲染浮层。只有一行的浮层多花一次点击，却没有任何可选项。这条规则是一个谓词（`addIsTheOnlyEntry`）覆盖两处表层，而不是逐表层特判。

由该规则派生出两条边界，它们同属这个决定：

- **列表为空只有在所需基线落地后才算最终结果。** 本地 Workspace 或远程根来源仍在等待时，菜单保留加载状态，而不是跳进一个即将到达的目标会使其变得多余的本地流程。仅添加表层不列已有本地 Workspace，但仍会等待远程根。
- **目录流程的洞没有占用者时，只移除本地添加操作。** Provider 持有的远程根仍可选择，也会让侧边栏按钮继续存在。目录流程与远程根都不存在时，侧边栏才不渲染按钮，而不是留下一个点了没反应的按钮；主视觉区菜单继续列出已有内容，没有内容时则什么也不弹。主视觉区的锚点 chip 归 ui-conversation 所有，因此本包能压掉浮层，却无法隐藏该 chip。
...
**为克隆仓库等不受支持的来源类型保留菜单壳。** 否决，依据「require a current owner and need」：菜单只为当前的本地或 Provider 目标出现，不承载假设中的入口。
...
- 没有 directory-picker 包的组合无法添加本地 Workspace；Provider 持有的远程根仍可选择，只有两类来源都不存在时才不渲染按钮。
