...
The current Client assembly explicitly mounts Commands, Goals, token-free Cohub Account operations, Cohub Space/files, read-only Cohub Board inspection, dynamic Cordis, read-only Host plugin inventory, and message-feedback Remote contributions. Cordis effect ownership withdraws every contribution when this assembly unloads, while `@deepseek-ai/dsh-api-gateway/client` owns descriptor validation, traced namespace Services, direct and scoped methods, invocation, and cancellation. The Client entry consumes the shared `TypertClientRemote` interface through Cordis and does not import the concrete Gateway. It re-exports the Gateway Client face's declaration merges type-only, so a consumer reaching the forwarded-event vocabulary through this facade gains no runtime edge to the Gateway implementation.
