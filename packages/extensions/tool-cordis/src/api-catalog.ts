...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  {
    key: 'cohubAccount',
    summary: 'The only owner of Cohub account secrets and token refresh within one Host.',
    description: 'The only owner of Cohub account secrets and token refresh within one Host.',
    methods: [
      {
        signature: 'readonly snapshot: CohubAccountObservable = { getSnapshot: () => this.current, subscribe: (listener) => { this.listeners.add(listener) return () => { this.listeners.delete(listener) } }, }',
        description: 'Token-free account state observable for Host and browser adapters.',
        parameters: [],
      },
      {
        signature: '@Remote(\'getAccount\') getAccount(): CohubAccountSnapshot',
        description: 'Return the current browser-safe account state.',
        parameters: [],
        returns: 'Token-free state for browser consumers.',
        throws: ['When the service is disposed.'],
      },
      {
        signature: '@Remote(\'beginLogin\') async beginRemoteLogin(): Promise<CohubAccountSnapshot>',
        description: 'Start device login without exposing the private device code.',
        parameters: [],
        returns: 'The public authenticating state after Cohub accepts the device request.',
      },
      {
        signature: '@Remote(\'pollLogin\') async pollRemoteLogin(): Promise<CohubAccountSnapshot>',
        description: 'Advance device login once; the browser controls no tokens or credentials.',
        parameters: [],
        returns: 'The public state after this poll settles.',
      },
      {
        signature: '@Remote(\'cancelLogin\') cancelRemoteLogin(): CohubAccountSnapshot',
        description: 'Cancel the active device login and return the resulting public state.',
        parameters: [],
        returns: 'The restored anonymous or authenticated state.',
      },
      {
        signature: '@Remote(\'logout\') async logoutRemote(): Promise<CohubRemoteLogoutResult>',
        description: 'Clear the Host-owned session and report any remote revocation warning.',
        parameters: [],
        returns: 'The anonymous public state and an optional remote revocation warning.',
      },
      {
        signature: 'beginLogin(signal?: AbortSignal): Promise<CohubDeviceAuthorization>',
        description: 'Start one Cohub device authorization flow.',
      ...
      {
        signature: 'pollLogin(signal?: AbortSignal): Promise<CohubLoginPollResult>',
        description: 'Poll the active device authorization once.',
        parameters: [{ name: 'signal', description: 'Optional caller cancellation.' }],
        returns: 'The pending, slow-down, authenticated, or terminal authorization result.',
      },
      {
        signature: 'cancelLogin(): void',
        description: 'Cancel the active device authorization and restore the preceding account state.',
        parameters: [],
      },
      {
        signature: 'getAccessToken(): Promise<string>',
        description: 'Resolve a usable Cohub access token, refreshing the private session when required.',
        parameters: [],
      ...
      {
        signature: 'logout(): Promise<CohubLogoutResult>',
        description: 'Clear the local private session before attempting remote token revocation.',
        parameters: [],
  ...
  {
    key: 'cohubBoard',
    summary: 'The only capability is exact, authenticated Board inspection.',
    description: 'The only capability is exact, authenticated Board inspection.',
    methods: [
      {
        signature: '@Remote(\'getBoard\') getBoard(spaceId: string, boardId: string): Promise<CohubBoardSnapshot>',
        description: 'Read one authenticated Cohub Board with its nodes and connections.',
        parameters: [{ name: 'spaceId', description: 'Owning Cohub Space id.' }, { name: 'boardId', description: 'Board id within that Space.' }],
  ...
  {
    key: 'cohubSpaces',
    summary: 'Typed Remote service backed only by Cohub account tokens and platform HTTP.',
    description: 'Typed Remote service backed only by Cohub account tokens and platform HTTP.',
    methods: [
      {
        signature: '@Remote(\'listSpaces\') listSpaces(): Promise<readonly CohubSpaceView[]>',
        description: 'List all Spaces accessible to the current account.',
        parameters: [],
      ...
      {
        signature: '@Remote(\'getConversation\') getConversation(spaceId: string, sessionId: string): Promise<CohubConversationView>',
        description: 'Read all currently retained Turns for one Cohub Session.',
        parameters: [{ name: 'spaceId', description: 'Cohub Space identity.' }, { name: 'sessionId', description: 'Cohub Session identity.' }],
      ...
      {
        signature: '@Remote(\'bindDshSession\') bindDshSession(spaceId: string, dshSessionId: string): Promise<CohubDshSessionBinding>',
        description: 'Attach one Cohub Space to an existing blank DSH Session.',
        parameters: [{ name: 'spaceId', description: 'Cohub Space identity.' }, { name: 'dshSessionId', description: 'Blank DSH Session identity.' }],
        returns: 'The installed binding.',
      },
      {
        signature: '@Remote(\'listDirectory\') listDirectory(spaceId: string, path: string): Promise<CohubSpaceDirectory>',
        description: 'List one exact Space-relative directory.',
        parameters: [{ name: 'spaceId', description: 'Cohub Space identity.' }, { name: 'path', description: 'Space-relative directory path.' }],
        returns: 'The exact directory listing.',
      },
      {
        signature: '@Remote(\'readText\') readText(spaceId: string, path: string): Promise<CohubSpaceTextFile>',
        description: 'Read one inline UTF-8 text file.',
        parameters: [{ name: 'spaceId', description: 'Cohub Space identity.' }, { name: 'path', description: 'Space-relative file path.' }],
        returns: 'The file content and revision.',
      },
      {
        signature: '@Remote(\'writeText\') writeText(spaceId: string, path: string, content: string, ifRevision: string): Promise<CohubSpaceWriteResult>',
        description: 'Compare-and-set one UTF-8 text file.',
        parameters: [{ name: 'spaceId', description: 'Cohub Space identity.' }, { name: 'path', description: 'Space-relative file path.' }, { name: 'content', description: 'Replacement UTF-8 content.' }, { name: 'ifRevision', description: 'Required current revision.' }],
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  {
    name: 'cohub-spaces/changed',
    mode: 'emit',
    signature: '\'cohub-spaces/changed\'(): void',
    summary: 'The account state changed; browser-side Space sources should refresh.',
    description: 'The account state changed; browser-side Space sources should refresh.',
    parameters: [],
  },
  {
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'CohubDeviceAuthorization',
    declaration: 'export interface CohubDeviceAuthorization {\n    readonly userCode: string;\n    readonly verificationUri: string;\n    readonly verificationUriComplete: string;\n    readonly expiresAt: number;\n    readonly retryAfterMs: number;\n}',
  },
  {
    name: 'CohubDshSessionBinding',
    declaration: 'export interface CohubDshSessionBinding {\n    readonly spaceId: string;\n    readonly spaceTitle: string;\n    readonly dshSessionId: string;\n}',
  },
  {
    name: 'CohubDshSessionStart',
    declaration: 'export interface CohubDshSessionStart {\n    readonly spaceId: string;\n    readonly cwd: string;\n}',
  },
  {
    name: 'CohubLoginPollResult',
    declaration: 'export type CohubLoginPollResult = {\n    readonly status: \'pending\';\n    readonly retryAfterMs: number;\n} | {\n    readonly status: \'authenticated\';\n    readonly profile: CohubAccountProfile;\n};',
  },
  {
  ...
  },
  {
    ...
    declaration: 'export interface CohubRemoteLogoutResult extends CohubLogoutResult {\n    readonly snapshot: CohubAccountSnapshot;\n}',
  },
  {
    name: 'CohubSessionView',
    declaration: 'export interface CohubSessionView {\n    readonly id: string;\n    readonly spaceId: string;\n    readonly title: string;\n    readonly status: string;\n    readonly latestMessageText?: string;\n    readonly updatedAt: string;\n}',
  },
  {
    name: 'CohubSpaceDirectory',
    declaration: 'export interface CohubSpaceDirectory {\n    readonly spaceId: string;\n    readonly path: string;\n    readonly entries: readonly CohubSpaceEntry[];\n}',
  },
  {
    name: 'CohubSpaceEntry',
    declaration: 'export interface CohubSpaceEntry {\n    readonly path: string;\n    readonly name: string;\n    readonly kind: \'folder\' | \'file\' | \'link\';\n    readonly size: number;\n    readonly revision: string;\n}',
  },
  {
    name: 'CohubSpaceSessionList',
    declaration: 'export interface CohubSpaceSessionList {\n    readonly spaceId: string;\n    readonly sessions: readonly CohubSessionView[];\n}',
  },
  {
    name: 'CohubSpaceTextFile',
    declaration: 'export interface CohubSpaceTextFile {\n    readonly spaceId: string;\n    readonly path: string;\n    readonly content: string;\n    readonly revision: string;\n}',
  },
  {
    name: 'CohubSpaceView',
    declaration: 'export interface CohubSpaceView {\n    readonly id: string;\n    readonly title: string;\n}',
  },
  {
    name: 'CohubSpaceWriteResult',
    declaration: 'export type CohubSpaceWriteResult = {\n    readonly ok: true;\n    readonly value: CohubSpaceTextFile;\n} | {\n    readonly ok: false;\n    readonly error: {\n        readonly code: \'version-conflict\';\n        readonly current: CohubSpaceTextFile;\n    };\n};',
  },
  {
    name: 'CohubTurnView',
