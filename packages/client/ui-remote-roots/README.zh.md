...
面向 DSH 的匿名、与供应方无关的远程根和会话界面。它把带来源标记的远程根放在本地 Workspace 旁边显示，并保持所有 Provider 身份不透明。每个具备能力的根都会明确提供 **Provider Agent** 与 **DSH Agent** 操作。Provider Session 行打开由 Provider 持有的会话；可交互来源会显示独立输入框、近实时轮询和取消控制，本地操作仍进入普通 DSH 工作台。本包不会创建账号或凭据，也不会虚构本地路径。
