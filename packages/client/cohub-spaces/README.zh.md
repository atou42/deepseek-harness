...
面向 `@deepseek-ai/dsh-client-remote-roots` 的浏览器侧 Cohub Provider。每个可访问 Space 都会成为带 `Cohub` 标识的根；展开后列出该 Space 内的 Cohub Session，而不是文件。Session 标识仍由 Provider 持有且保持不透明。所有操作都经过类型化 Host Remote 适配器，因此凭证不会进入本包或浏览器状态。
