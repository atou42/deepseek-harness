...
Browser-side Cohub provider for `@deepseek-ai/dsh-client-remote-roots`. Each accessible Space becomes a root marked `Cohub`; expanding it lists that Space's Cohub Sessions, never its files. Session identifiers remain provider-owned opaque values. All operations travel through the typed Host Remote adapter, so credentials never enter this package or browser state.
