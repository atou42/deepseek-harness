...
| `ctx.cohubAccount` | `core` | [`cohub-account`](../packages/identity/cohub-account) | - | [`llm-cohub`](../packages/llm/llm-cohub), [`cohub-generation`](../packages/generation/cohub-generation), [`cohub-spaces`](../packages/host/cohub-spaces), [`cohub-board`](../packages/host/cohub-board) | - | Owns device authorization, private token persistence, refresh, and logout; Cohub adapters request access tokens without exposing them to the browser. |
| `ctx.cohubSpaces` | `core` | [`cohub-spaces`](../packages/host/cohub-spaces) | - | - | - | Exposes authenticated Space, Session, conversation, file, command, and DSH Session-binding operations over the Host remote boundary. |
| `ctx.cohubBoard` | `core` | [`cohub-board`](../packages/host/cohub-board) | - | - | - | Reads validated Board snapshots through the Cohub account owner without adding edit or publish capabilities. |
