...
flowchart LR
  ...
  pkg_cohub_account["cohub-account"]
  svc_cohubAccount["ctx.cohubAccount<br/>Cohub account owner"]
  pkg_llm_cohub["llm-cohub"]
  pkg_cohub_generation["cohub-generation"]
  pkg_cohub_spaces["cohub-spaces"]
  pkg_cohub_board["cohub-board"]
  svc_cohubSpaces["ctx.cohubSpaces<br/>Cohub Space adapter"]
  svc_cohubBoard["ctx.cohubBoard<br/>Cohub Board adapter"]
  ...
  pkg_cohub_account --> svc_cohubAccount
  pkg_cohub_board --> svc_cohubBoard
  pkg_cohub_spaces --> svc_cohubSpaces
  ...
  svc_cohubAccount --> pkg_cohub_board
  svc_cohubAccount --> pkg_cohub_generation
  svc_cohubAccount --> pkg_cohub_spaces
  svc_cohubAccount --> pkg_llm_cohub
...
| `ctx.cohubAccount` | `core` | [`cohub-account`](../packages/identity/cohub-account) | - | [`llm-cohub`](../packages/llm/llm-cohub)、[`cohub-generation`](../packages/generation/cohub-generation)、[`cohub-spaces`](../packages/host/cohub-spaces)、[`cohub-board`](../packages/host/cohub-board) | - | 负责设备授权、私密令牌持久化、刷新和登出；Cohub 适配器可请求访问令牌，但不会将其暴露给浏览器。 |
| `ctx.cohubSpaces` | `core` | [`cohub-spaces`](../packages/host/cohub-spaces) | - | - | - | 通过 Host 远程边界暴露经过认证的 Space、Session、对话、文件、命令与 DSH Session 绑定操作。 |
| `ctx.cohubBoard` | `core` | [`cohub-board`](../packages/host/cohub-board) | - | - | - | 通过 Cohub 账户服务读取经过校验的 Board 快照，不额外提供编辑或发布能力。 |
